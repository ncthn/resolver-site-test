...
const LOGO_W = '/logo/recolor/oct-whiteblue-t.png' // white mark — dark backgrounds
const LOGO_B = '/logo/recolor/oct-bluewhite-t.png' // blue mark — light backgrounds
...
/* ---------- interactive inbox ---------- */
type Ticket = { id: string; red: boolean; title: string; who: string; q: string; esc: boolean; head: string; meta: string; body: string }
const TICKETS: Ticket[] = [
  { id: 't1', red: true, title: 'Chargeback threatened', who: 'A. Weber · DE', q: '"I\'m reporting this to my bank and my lawyer."', esc: true, head: 'Held for a human — never auto-sent', meta: '#1991 · disputed · $59', body: 'Hi A. Weber — I\'m sorry about order #1991. I\'ve escalated this to a specialist who will personally review the dispute and reply within 24 hours.' },
  { id: 't2', red: false, title: 'Where is my order?', who: 'Maria Lopez · US', q: '"I ordered 3 weeks ago and still nothing 😟"', esc: false, head: 'Auto-drafted · 0.8s', meta: '#1042 · in transit · 21 days', body: 'Hi Maria — your order #1042 shipped and is in transit, arriving in 2–3 days. Here\'s live tracking: CP998… I\'ll keep an eye on it.' },
  { id: 't3', red: false, title: 'Return request', who: 'James Carter · GB', q: '"It didn\'t fit — can I return it?"', esc: false, head: 'Auto-drafted · policy-aware', meta: '#2090 · delivered · within window', body: 'Hi James — happy to help with your return. Here\'s your prepaid label and the 3 quick steps to send it back.' },
]
function InboxDemo() {
      ...
      <div className="win-body">
        ...
        <div className="win-pane">
          ...
          <div key={t.id} className={'draft' + (t.esc ? ' esc' : '')}>
            <div className="dh" style={{ color: t.esc ? '#B23636' : 'var(--teal)' }}>{t.esc ? <Gavel size={11} /> : <Sparkles size={11} />}{t.head}</div>
            <div className="bd">{t.body}</div>
            <div style={{ fontSize: 11, color: '#9A9AB0', marginTop: 10 }}>{t.meta}</div>
            <div className="act">{t.esc ? (<><span className="mini rv">Review</span><span className="mini gh">Edit</span></>) : (<><span className="mini go">Approve &amp; send</span><span className="mini gh">Edit</span></>)}</div>
          </div>
        </div>
      </div>
    </div>
...
function LanguagesDemo() {
  ...
  return (
    <>
      <div className="langtabs">{['EN', 'FR', 'DE', 'ES'].map((l) => <span key={l} className={'tb' + (l === lang ? ' on' : '')} onClick={() => setLang(l)}>{l}</span>)}</div>
...
/* ---------- pricing ---------- */
type Plan = { n: string; d: string; m: number | null; rec: boolean; f: string[] }
const PLANS: Plan[] = [
  { n: 'Solo', d: '1 store · 1 seat · 300 tickets/mo', m: 59, rec: false, f: ['Shadow + auto-send', 'Live order & tracking lookup', 'Chargeback & legal flags', '30+ languages'] },
  { n: 'Team', d: '3 stores · 3 seats · 2,500 tickets/mo', m: 249, rec: true, f: ['Everything in Solo', 'Per-store voice & policy', 'Lane analytics', 'Priority email support'] },
  { n: 'Portfolio', d: 'Unlimited stores · 10 seats · 6,000/mo', m: 599, rec: false, f: ['Everything in Team', 'Cross-store insights', 'Custom rules engine', 'Priority support'] },
  { n: 'Enterprise', d: 'Custom volume & contract', m: null, rec: false, f: ['Unlimited everything', 'SSO & data residency', 'Dedicated success manager', 'SLA & security review'] },
]
function Pricing() {
...
const FAQS: [string, string][] = [
  ...
  ['How long does setup take?', 'About 10 minutes. Install from Shopify, connect your support inbox, and Resolver starts drafting grounded replies immediately in shadow mode.'],
  ['Does it really use live order data?', 'Yes — every reply is written against the real Shopify order, fulfilment status and live carrier tracking, so answers are accurate, not generic.'],
  ['What languages are supported?', "30+. Resolver detects the customer's language, replies natively, and shows your team the English version alongside."],
  ['Can I run multiple stores?', 'Yes — every storefront flows into one shared inbox, each with its own voice, policy and shadow/auto setting. Filter to one or work them all at once.'],
]
function Faq() {
      ...
      <div className="faq">
        {FAQS.map(([q, a], i) => (
          <div key={i} className={'qa reveal' + (open === i ? ' open' : '')}>
            <div className="q" onClick={() => setOpen(open === i ? -1 : i)}>{q}<Plus className="pl" size={20} /></div>
    ...
    <div id="top" ref={ref}>
      ...
      <section className="band c-white trust raised">
        <div className="wrap">
          ...
          <div className="stats">{STATS.map(([n, l, t], i) => <div className="stat reveal" key={i}><div className={'n' + (t ? ' t' : '')}>{n}</div><div className="l">{l}</div></div>)}</div>
        </div>
      </section>

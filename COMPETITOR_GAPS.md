...
|---|---|---|
| Carrier tracking layer — demo shipped (proactive-draft log + source rows); production needs the 17TRACK account | Repline (ParcelWILL), Humane (DHL/DPD/Hermes/UPS) | Already researched: 17TRACK webhooks (TRACKING_RESEARCH.md). Unlocks proactive drafts on "delivery failed". |
...
| Macros / saved replies — SHIPPED in test app 2026-07-06 | Gorgias (macros with variables) | Compose has templates; the ticket view has none. Small feature, daily-use value. |
| CSAT survey — SHIPPED in test app 2026-07-06 | Gorgias | One-click rating appended after resolution; feeds lane trust and the marketing site (honest numbers eventually). |
| Pre-reply check surfaced in UI — SHIPPED in test app 2026-07-06 | Humane ("escalation prevention") | Production already runs draftSanityCheck; the app never shows it. Cheap parity: a "checks passed" line on the draft (we show trace; add the sanity verdict). |
